...
//https://github.com/rsdoiel.atom
$(document).ready(function () {
  /* Grab my shared items from Google Reader. */
  var urls = { 'google_reader': 'http://www.google.com/reader/public/javascript/user/05065097874671388976/state/com.google/broadcast?n=17',
  ...
  $.getJSON(urls.google_reader + '&callback=?', function (data) {
  ...
  $.getJSON(urls.github_rsdoiel + '?callback=?', function (data) {  
  	var i = 0, repos = {};
    if (data.length > 0) {
        ...
        $('#github-my-repos').append(makeGithubList(repos));
    } else {
        $('#github-my-repos').append("Can't reach github.com");
    }
  });

  /* urls.github_uscwebservices_repos */
  $.getJSON(urls.github_uscwebservices + '?callback=?', function (data) {  
  	var i = 0, repos = {};
    if (data.length > 0) {
    	for (i = 0; i < data.length; i += 1) {
		if (data[i].repository !== undefined &&
        ...
        $('#github-uscwebservices-repos').append(makeGithubList(repos));
    } else {
        $('#github-uscwebservices-repos').append("Can't reach github.com");
    }
  });
});
