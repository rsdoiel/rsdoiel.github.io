...
One of thing I have in my toolbox is a static site web server. It amazes me how often I wind up using on. PHP and Python can launch one easily from the command line. What I've found though is they are some what lacking. What I want is a simple web server that runs only on localhost. I should be able set choose the port it runs no. I should be able to specify a document root for the content I want to expose. It should correctly handle the common content types, e.g. JavaScript files as "application/json" not "text/plain".
